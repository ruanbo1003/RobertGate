...
3. 已登录用户访问首页，通过导航栏进入受保护的 AI Tools 区域
...
- [ ] 已登录/未登录用户均可访问 `/`，页面内容一致（不做分流跳转）
- [ ] 导航栏已登录状态：Sign In 按钮替换为头像下拉菜单（详见 `auth.md#用户菜单`）
...
- 已登录用户访问 `/`：正常显示 landing，通过 Navbar 头像菜单查看用户信息或进入 AI Tools
- 未登录用户点击 AI Tools 卡片：跳转 `/login`，登录成功后回到 `/ai-tools`
- 导航收藏 / 个人简介未实现的子模块：路由占位显示 Not Implemented 页面
- 移动端横屏：卡片保持单列，不做特殊处理
...
| 路由 | 页面 | 是否需要登录 | 说明 |
|------|------|-------------|------|
| `/` | HomePage | 否 | landing 页面，登录/未登录均可访问 |
| `/ai-tools/*` | AiToolsLayout + 子页面 | 是 | 受 `ProtectedRoute` 保护 |
| `/bookmarks` | NotImplementedPage | 否 | 占位 |
| `/about` | AboutPage | 否 | 占位 |
