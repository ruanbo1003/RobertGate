...
from app.application.dto.t2i import (
    ImageOut,
    TaskDetail,
    TaskSummary,
    TemplateBrief,
...
class T2ITaskService:
    ...
    async def list_templates(self) -> dict:
        ...
        return {"templates": [TemplateOut.model_validate(t) for t in rows]}
    ...
    async def list_tasks(
        ...
        for t in tasks:
            ...
            items.append(TaskSummary.build(t, imgs))

        return {
            "template": TemplateBrief.build(tpl),
            "page": page,
            "page_size": page_size,
            "total": total,
            "items": items,
    ...
    async def create_task(
        ...
        if existing is not None:
            ...
            return {"existing": True, "task": TaskSummary.build(existing, imgs)}
        ...
        return {"existing": False, "task": TaskSummary.build(task, [image])}
    ...
    async def get_task(self, task_id: str) -> dict:
        ...
        return {
            "task": TaskDetail.build(task, images),
            "images": [ImageOut.build(i) for i in images],
        }
    ...
    async def retry_task(self, task_id: str) -> dict:
        ...
        return {"image": ImageOut.build(image)}
